...
def DeleteData():
    if not tree.selection():
       result = tkMessageBox.showwarning('Input Validation', 'Please Select Something First!', icon="warning")
    else:
        result = tkMessageBox.askquestion('Delete Records', 'Are you sure you want to delete this record?', icon="warning")
        if result == 'yes':
            curItem = tree.focus()
            contents =(tree.item(curItem))
            selecteditem = contents['values']
...
def UpdateExistingWindow():
    ...
    if not tree.selection():
       result = tkMessageBox.showwarning('', 'Please Select Something First!', icon="warning")
        ...
        #===================LABELS==============================
        lbl_title = Label(FormTitle, text=f"Updating Member: {mem_id}", font=('arial', 16), bg="#66ff66",  width = 300)
        lbl_title.pack(fill=X)
        lbl_firstname = Label(ContactForm, text="Firstname", font=('arial', 14), bd=5)
        lbl_firstname.grid(row=0, sticky=W)
        lbl_lastname = Label(ContactForm, text="Lastname", font=('arial', 14), bd=5)
        ...
        #==================BUTTONS==============================
        btn_addcon = Button(ContactForm, text="Save", width=50, command=UpdateData)
        btn_addcon.grid(row=6, columnspan=2, pady=10)
...
MidCenter = Frame(Mid, width=100)
MidCenter.pack(side=LEFT, padx=10)

MidCenterSearch = Frame(Mid, width=100)
MidCenterSearch.pack(side=LEFT,padx=10)
